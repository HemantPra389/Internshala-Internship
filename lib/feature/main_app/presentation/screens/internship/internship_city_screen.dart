...
class InternshipCityScreen extends StatefulWidget {
  const InternshipCityScreen({super.key});

  @override
  State<InternshipCityScreen> createState() => _InternshipCityScreenState();
}

class _InternshipCityScreenState extends State<InternshipCityScreen> {
  var city_chip_list = Hive.box(AppConstants.cityBox);
  late List<int> chipTagsList = [];
  List<String> filteredCityList = AppConstants.cities;
  ...
  Future getChipData() async {
    if (chipTagsList.isEmpty) {
      chipTagsList = city_chip_list.get('key') ?? [];
    }
  }
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          GestureDetector(
            onTap: () {
              Navigator.of(context).pop();
              city_chip_list.put('key', chipTagsList);
            },
            child: Container(
        ...
        child: Column(
          children: [
            TextField(
              ...
              onChanged: (value) {
                setState(() {
                  filteredCityList = AppConstants.cities
                      .where((city) => city
                          .toLowerCase()
